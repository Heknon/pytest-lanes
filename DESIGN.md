...
Also documented: F17 (`PYTEST_CURRENT_TEST` is per process).
...
| F17 | **`PYTEST_CURRENT_TEST` is per process** (an environment variable cannot differ per thread): a test, or a subprocess it starts (which inherits the environment), can read another lane's test name or none (4–5 of 6 checks under load). Use `request.node.nodeid` in the test, or pass the name to the subprocess explicitly. The failure-instrumentation plugin does not read it. |
