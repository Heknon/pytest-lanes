...
Touchpoints are probed at startup (`probes.py`), and the plugin fails closed if a probe fails. P1 has no startup probe; the contract tests cover it. Each patch lives in the module named in CLAUDE.md's file map, with its install and restore in one context manager.
...
| P3 | `LoggingPlugin.caplog_handler` / `report_handler` / `log_level`; `catching_logs.__enter__` | Per-lane log capture; non-propagating loggers routed as the installed pytest captures them |
| P4 | pluggy `_inner_hookexec` | Routes controller hooks to the main thread |
| P5 | `item._nodeid`; xdist's `WorkerInteractor.pytest_collection_modifyitems` | Adds the `@group` suffix under loadgroup with the installed xdist's own code |
...
| P7 | `config._tmp_path_factory` / `_tmpdirhandler` | A basetemp per lane, like xdist's per-worker basetemp; the process's basetemp is created under a lock. pytest's `cleanup_dead_symlinks` runs on lane basetemps when it exists |
...
| X4 | `WorkerController.workerinput` / `workerinfo` / `workeroutput` / `shutting_down` | Hybrid mode only; mirrored on each `LaneProxy` so custom schedulers see worker-shaped nodes, and never sent work once the worker is down or draining |
...
| P15 | `CaptureManager.suspend_global_capture` / `resume_global_capture` | On a lane they also suspend the test's capture fixture, so a plugin's terminal output (`--setup-show`) is not captured by the test's capfd (and `CaptureFixture._is_started`) |
...
| F5 | **Crash collateral:** in hybrid mode, every in-flight test of a crashed process is reported crashed, as xdist reports its crashed test, because which lane crashed it cannot be told apart. What happens next is the scheduler's: under a loadscope-based scheduler (loadscope, loadgroup, `EnvScheduling`) the unfinished environments are requeued whole, starting at the step that was running, so each of those tests is run again, in order, on one lane. Under `--dist load` they are not rerun, a sibling lane's next queued test is reported crashed and dropped (also when a test ends its worker with `pytest.exit`, which xdist treats as a crash), and a test whose reports were sent but whose completion was not is reported crashed after passing and run again |
...
| F8 | **Ctrl-C** interrupts every lane and runs its teardown; a lane blocked in one long C call cannot be interrupted and is abandoned after `lanes_interrupt_grace`, named. KeyboardInterrupt is raised in a lane only in the test's own code: at a call from its code into its code (a `sys.monitoring` PY_START callback), or, for a lane blocked in a C call in its own code, with `PyThreadState_SetAsyncExc` as that call returns. Raised anywhere else it could land between a lock's acquire and its release (on 3.14 even `with lock:` has such a point; logging's `_releaseLock()` is a Python call inside a `finally`), and that lock stayed held for every other lane: 17 of 20 interrupted runs of 32 lanes hung on 3.14t, 30 of 30 on 3.12. A lane running only library code (the standard library, pytest, pluggy, xdist, execnet) is not interrupted: it finishes its current test, then stops. SIGTERM kills the process without teardown, as it does plain pytest |
