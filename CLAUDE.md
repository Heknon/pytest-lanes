...
4. **Plugins that observe xdist must keep working.** Terminal, junitxml, report-log, pytest-html, pytest-metadata, pytest-cov, rerunfailures, and the user's own failure-instrumentation plugin. In single-process mode every lane is a node for xdist's controller hooks, fired to every plugin in DSession's order (`pytest_configure_node`, `pytest_testnodeready`, `pytest_xdist_node_collection_finished`, `pytest_testnodedown`), with the `workerinput`/`workeroutput` protocol working through each lane; `--lanes` alone must look like `-n` to such plugins.
...
| `runner.py` | `LaneRunner`, the base of both test-running sessions: session-long install/uninstall, per-phase capture, the lane loop (`_node_loop`, a copy of xdist's `WorkerInteractor` loop), the main-thread pump (`_pump`), Ctrl-C handling (`_interrupt`: `LaneInterrupter` raises `KeyboardInterrupt` in each lane only where no library lock can be held (a `sys.monitoring` PY_START callback, at a call from the test's own code into its own code; `PyThreadState_SetAsyncExc` for a lane blocked in a C call in its own code), teardown on the lane, `lanes_interrupt_grace`), and `ReadWriteLock` for exclusive tests | P9 |
| `single.py` | `SingleProcessSession` (`--lanes N`): plays xdist's DSession on the main thread (scheduler and node hooks), then runs exclusive tests on `ln-serial` | |
...
| `compat.py` | Shims for third-party plugins that assume one test at a time per process, or a worker per node | C1, C2, C3, C4 |
...
| C3 | pytest-metadata: `metadata_key` (its stash key) | Its worker-side `pytest_configure` writes `config.workeroutput["metadata"]`, which its controller-side `pytest_testnodedown` reads from each node; a lane never runs a worker-side configure, so each lane's `workeroutput` is seeded with it (`compat.seed_worker_output`). Without it, testnodedown raised KeyError in single-process mode. Probed |
| C4 | pytest-cov: `_cov` plugin's `cov_controller` | In one process pytest-cov measures with its local engine (`Central`), which traces every lane's thread; its xdist node hooks belong to its distributed engine and fail on the local one (`'Central' object has no attribute 'configure_node'`). Lanes' node hooks skip pytest-cov exactly then (`compat.node_hook_callers`), as plain pytest-cov never sees node hooks without `-n`. Probed |
...
- **Ctrl-C** interrupts every lane and runs its teardown, as pytest does, but only in the test's own code (never inside the standard library, pytest, pluggy, xdist or execnet, where it could leave a lock held for good): a lane running only library code finishes its current test first. A lane blocked in one long C call (a sleep, a socket read without timeout) is abandoned after `lanes_interrupt_grace` (30s) and named. SIGTERM kills the process without teardown, in every mode.
